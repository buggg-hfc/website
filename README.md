...
一个可部署在 Ubuntu 云服务器上的 Node.js + 原生前端个人网站，涵盖「学习博客」「书影音剧游条目」「日常动态」三个板块，并内置内容管理后台，可在浏览器中维护所有数据。

- **学习板块**：以博客文章卡片展示，可在 `data/blog.json` 配置标题、日期、标签、摘要及正文段落。
- **书影音剧游板块**：按书籍、电影、番剧、音乐、游戏、戏剧分类展示。条目数据存放在 `data/media.json`，支持配置个人评分与外部链接（豆瓣 / IMDb / IGN）。服务端会在有链接时自动抓取平台评分与热门短评，缓存 1 小时。
...
访问 `http://localhost:3000` 查看网站，访问 `http://localhost:3000/admin` 打开管理后台。
...
- **书影音剧游条目**：编辑 `data/media.json`。每个条目允许填写 `links`，只要提供对应平台链接，服务端就会尝试爬取评分与热门短评；若抓取失败，将自动降级并保留本地信息。
